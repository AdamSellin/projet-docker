   ...
   <head>
    ...
    <nav>
		  <ul>
            ...
            if (isset($_POST['mdp']) && !empty($_POST['mdp']) && isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['nom']) && !empty($_POST['nom']) && isset($_POST['prenom']) && !empty($_POST['prenom']) && isset($_POST['date_naissance']) && !empty($_POST['date_naissance']) && isset($_POST['telephone']) && !empty($_POST['telephone'])) {
                
                //Insertion avec recherche de doublon
                ...
                if ($req->execute(array('lid' => $_POST['identifiant'], 'lemdp' => $_POST['mdp'], 'lemail' => $_POST['email'], 'lenom' => $_POST['nom'], 'lepnom' => $_POST['prenom'], 'ladatenaissance' => $_POST['date_naissance'], 'letelephone' => $_POST['telephone']))) {
                    ...
                    if ($res == '') {
                        // Modification du profil dans la bdd
                        $req = $linkpdo->prepare('UPDATE `profils` SET `mdp`="'.$_POST['mdp'].'", `email`="'.$_POST['email'].'", `nom`="'.$_POST['nom'].'", `prenom`="'.$_POST['prenom'].'", `date_naissance`="'.$_POST['date_naissance'].'", `telephone`="'.$_POST['telephone'].'" WHERE `identifiant`="'.$_SESSION['identifiant'].'" ') ;
                        
                        $req->execute() ;

                        echo '<p class="texteAjout">Le profil a été modifié.</p>' ;
                    } else {
